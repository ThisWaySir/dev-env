...
" 对部分语言设置单独的缩进
au FileType scala,clojure,elixir,eelixir,lfe,scheme,racket,newlisp,lisp,lua,ruby,eruby,julia,dart,elm,coffee,ls,slim,jade,yaml,sh,vue set shiftwidth=2
au FileType scala,clojure,elixir,eelixir,lfe,scheme,racket,newlisp,lisp,lua,ruby,eruby,julia,dart,elm,coffee,ls,slim,jade,yaml,sh,vue set tabstop=2
...
au BufRead,BufNewFile *.vue      setlocal ft=html
...
set nolist                   " 显示特殊字符，其中Tab使用高亮~代替，尾部空白使用高亮点号代替
...
let g:go_auto_type_info                   = 1  " 关闭Go语言自动显示类型信息 [默认就是关闭的，此处用于方便需要时开启]
